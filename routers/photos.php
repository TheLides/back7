    ...
    if ($mysqli->connect_error) {
        ...
        if (count($urlData) == 0) {
            ...
            if ($perm != 0) {
                ...
                printjsonPhotos($mysqli->query("SELECT * FROM `photo` WHERE `photo`.`UserId` = '$userId'")) or die(mysqli_error($mysqli));
            } else {
...
function post($mysqli, $urlData)
{
    if ($mysqli->connect_error) {
        ...
        $perm = checkPermission($headers["Authorization"]);
        if ($perm != 0) {
            $user = mysqli_fetch_array($mysqli->query("SELECT Id FROM `user` WHERE `user`.`token` = '$UserToken'"))[0] or die(mysqli_error($mysqli));

            if ($_FILES && $_FILES["filename"]["error"] == UPLOAD_ERR_OK ) {
                $name = htmlspecialchars(basename($_FILES["File"]["name"]));
                $path =  "uploads/" . time() . $name;
                if (move_uploaded_file($_FILES["File"]["tmp_name"], $path)) {
                    $mysqli->query("INSERT INTO `photo` (`id`, `userid`, `link`) VALUES (NULL, '$user', '$path')") or die(mysqli_error($mysqli));
                    header('HTTP/1.0 200 OK');
                    echo json_encode($mysqli->query("SELECT * FROM `photo` WHERE `photo`.`UserId` = '$user' AND `photo`.`Link` = '$path'")) or die(mysqli_error($mysqli));
                    return;
                } else {
    ...
    if ($mysqli->connect_error) {
        ...
        if ($per == 1 || $UserId == $urlData[0]) {
            if (count($urlData) == 1) {
                ...
                $mysqli->query("DELETE FROM `photo` WHERE `photo`.`Id` = '$urlData[0]'") or die(mysqli_error($mysqli));
                header('HTTP/1.0 200 OK');
                return;
            }
        } else {
